...
export default function AccountHeader() {
  ...
  return (
    <header className={ scroll ? "header-flex sticky" : "header-flex" }>
      ...
      <div className="account-header-icons-margin">
        <Support show={show} showTitle={showTitle} setShowTitle={setShowTitle}/>
        <HeaderIcons />
      </div>
    </header>
  )
...
const AccountLogo = () => (
  <div className="header-logo">
    ...
    <div className="header-account-text">{t('gapps.account')}</div>
  </div>
)

const Support = ({ show, showTitle, setShowTitle }) => (
  <a 
    ...
    <SupportTitle showTitle={showTitle} />
  </a>
)
